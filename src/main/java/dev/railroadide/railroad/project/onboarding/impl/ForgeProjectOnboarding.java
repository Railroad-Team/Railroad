...
import dev.railroadide.railroad.form.FormComponentBuilder;
import dev.railroadide.railroad.form.ui.InformativeLabeledHBox;
import dev.railroadide.railroad.project.*;
import dev.railroadide.railroad.project.creation.ProjectCreationService;
...
public class ForgeProjectOnboarding {
    ...
    public void start(Scene scene) {
        var flow = OnboardingFlow.builder()
            ...
            .addTransition("minecraft_version", "forge_version")
            .addTransition("forge_version", "mapping_channel")
        ...
        return OnboardingFormStep.builder()
            ...
            .appendSection("railroad.project.creation.section.minecraft_version",
                described(
                    FormComponent
                        ...
                        .valueOfFunction(ForgeProjectOnboarding::getMinecraftVersion)
                        .required()
                        .translate(false),
            ...
            .onEnter(ctx -> {
                ...
                if (availableVersions.isEmpty() || now > nextInvalidationTime.get()) {
                    FORGE_MINECRAFT_VERSIONS_CACHE.getIfPresent().ifPresent(values -> Platform.runLater(() -> {
    ...
    private OnboardingStep createForgeVersionStep() {
        ObservableList<String> availableVersions = FXCollections.observableArrayList();
        StringProperty latestForgeVersion = new SimpleStringProperty();

        return OnboardingFormStep.builder()
            .id("forge_version")
            .title("railroad.project.creation.forge_version.title")
            .description("railroad.project.creation.forge_version.description")
            .appendSection("railroad.project.creation.section.forge_version",
                described(
                    FormComponent
                        .comboBox(ForgeProjectKeys.FORGE_VERSION, "railroad.project.creation.forge_version",
                            String.class)
                        .required()
                        .items(() -> availableVersions)
                        .defaultValue(() -> {
                            String latest = latestForgeVersion.get();
                        ...
                        .cellFactory(param -> new StarableListCell<>(
                            version -> isRecommendedForgeVersion(version, latestForgeVersion.get()),
                            latest -> Objects.equals(latest, latestForgeVersion.get()),
                            Function.identity()))
                        .buttonCell(new StarableListCell<>(
                            version -> isRecommendedForgeVersion(version, latestForgeVersion.get()),
                            latest -> Objects.equals(latest, latestForgeVersion.get()),
                            Function.identity())),
                    "railroad.project.creation.forge_version.info"))
            .onEnter(ctx -> {
                MinecraftVersion minecraftVersion = ctx.get(MinecraftProjectKeys.MINECRAFT_VERSION);

                fetchForgeVersions(minecraftVersion).whenComplete((payload, throwable) -> {
                    if (throwable != null) {
                        Railroad.LOGGER.error("Failed to fetch Forge versions for Minecraft {}", minecraftVersion,
                            throwable);
                        return;
                    }
                    ...
                    Platform.runLater(() -> {
                        ...
                        latestForgeVersion.set(payload.latest());
    ...
    private OnboardingStep createModDetailsStep() {
        ...
        return OnboardingFormStep.builder()
            ...
            .appendSection("railroad.project.creation.section.forge_options",
                described(
    ...
    private static CompletableFuture<List<MinecraftVersion>> resolveForgeMinecraftVersions() {
        return FORGE_MINECRAFT_VERSIONS_CACHE.getAsync(() -> SwitchboardRepositories.FORGE.getAllVersions()
            .thenApply(versions -> versions.stream()
                .map(ForgeProjectOnboarding::extractMinecraftVersionId)
                .flatMap(Optional::stream)
                .map(ForgeProjectOnboarding::lookupMinecraftVersion)
                .flatMap(Optional::stream)
                .distinct()
    ...
    private static CompletableFuture<ForgeVersionsPayload> fetchForgeVersions(MinecraftVersion version) {
        if (version == null)
            return CompletableFuture.completedFuture(new ForgeVersionsPayload(null, List.of(), null));
        ...
        return versionsFuture
            .thenCombine(latestFuture,
                (versions, latest) -> new ForgeVersionsPayload(version, versions == null ? List.of() : versions,
                    latest))
            .exceptionally(throwable -> {
                Railroad.LOGGER.error("Failed to fetch Forge versions for Minecraft {}", version, throwable);
                return new ForgeVersionsPayload(version, List.of(), null);
            });
    }
    ...
    private static boolean isRecommendedForgeVersion(String version, String latest) {
        return version != null && Objects.equals(version, latest) && !isForgePrerelease(version);
    }
