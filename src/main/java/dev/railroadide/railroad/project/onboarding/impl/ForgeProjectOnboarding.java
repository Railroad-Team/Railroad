...
public class ForgeProjectOnboarding {
        ...
        return OnboardingFormStep.builder()
            ...
            .onEnter(ctx -> {
                ...
                if (availableVersions.isEmpty() || now > nextInvalidationTime.get()) {
                    FORGE_MINECRAFT_VERSIONS_CACHE.getIfPresent().ifPresent(values ->
                        Platform.runLater(() -> {
    ...
    private OnboardingStep createForgeVersionStep() {
        ...
        return OnboardingFormStep.builder()
            ...
            .appendSection("railroad.project.creation.section.forge_version",
                described(
                    FormComponent.comboBox(ForgeProjectKeys.FORGE_VERSION, "railroad.project.creation.forge_version", String.class)
                        .required()
                        .items(() -> availableVersions)
                        .defaultValue(() -> {
            ...
            .onEnter(ctx -> {
                ...
                fetchForgeVersions(minecraftVersion).whenComplete((payload, throwable) -> {
                    if (throwable != null) {
                        Railroad.LOGGER.error("Failed to fetch Forge versions for Minecraft {}", minecraftVersion, throwable);
                        return;
                    }
    ...
    private static CompletableFuture<ForgeVersionsPayload> fetchForgeVersions(MinecraftVersion version) {
        ...
        return versionsFuture.thenCombine(latestFuture, (versions, latest) ->
                new ForgeVersionsPayload(version, versions == null ? List.of() : versions, latest))
