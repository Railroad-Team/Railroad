...
public final class CoreDefiniteAssignmentInspection implements JavaInspectionRuleProvider {
    ...
    private static FlowState analyzeNode(
        ...
        return switch (kindId) {
            ...
            case JAVA_LOCAL_VARIABLE_DECLARATION_STATEMENT ->
                analyzeLocalVariableDeclaration(context, node, state, unassignedReporter, illegalFinalReporter);
            case JAVA_IF_STATEMENT ->
                analyzeIfStatement(context, node, state, unassignedReporter, illegalFinalReporter);
            case JAVA_LABELED_STATEMENT ->
                analyzeLabeledStatement(context, node, state, unassignedReporter, illegalFinalReporter);
            case JAVA_WHILE_STATEMENT ->
                analyzeWhileStatement(context, node, state, unassignedReporter, illegalFinalReporter);
            case JAVA_DO_WHILE_STATEMENT ->
                analyzeDoWhileStatement(context, node, state, unassignedReporter, illegalFinalReporter);
            case JAVA_FOR_STATEMENT ->
                analyzeForStatement(context, node, state, unassignedReporter, illegalFinalReporter);
            case JAVA_SWITCH_STATEMENT, JAVA_SWITCH_EXPRESSION ->
                analyzeSwitchStatement(context, node, state, unassignedReporter, illegalFinalReporter);
            case JAVA_TRY_STATEMENT ->
                analyzeTryStatement(context, node, state, unassignedReporter, illegalFinalReporter);
            case JAVA_RETURN_STATEMENT, JAVA_THROW_STATEMENT, JAVA_BREAK_STATEMENT, JAVA_CONTINUE_STATEMENT,
                JAVA_YIELD_STATEMENT ->
    ...
    private static FlowState analyzeIfStatement(
        ...
        FlowState conditionState = analyzeNode(context, children.getFirst(), state, unassignedReporter,
            illegalFinalReporter);
        FlowState thenState = children.size() > 1
            ? analyzeNode(context, children.get(1), conditionState, unassignedReporter, illegalFinalReporter)
            : conditionState;
        FlowState elseState = children.size() > 2
            ? analyzeNode(context, children.get(2), conditionState, unassignedReporter, illegalFinalReporter)
            : conditionState;
    ...
    private static FlowState analyzeLabeledStatement(
        ...
        FlowState labeledState = analyzeLabeledTarget(context, target, label, state, unassignedReporter,
            illegalFinalReporter);
    ...
    private static FlowState analyzeLabeledTarget(
        ...
        return switch (target.kind().id()) {
            ...
            case JAVA_DO_WHILE_STATEMENT ->
                analyzeDoWhileStatement(context, target, state, unassignedReporter, illegalFinalReporter, label);
            case JAVA_FOR_STATEMENT ->
                analyzeForStatement(context, target, state, unassignedReporter, illegalFinalReporter, label);
    ...
    private static FlowState analyzeWhileStatement(
        ...
        FlowState loopHead = stabilizeWhileLoopHead(context, condition, body, state, unassignedReporter,
            illegalFinalReporter, loopLabel);
        FlowState conditionState = analyzeNode(context, condition, loopHead, unassignedReporter, illegalFinalReporter);
        FlowState bodyState = body == null
            ? conditionState
            : analyzeNode(context, body, conditionState, unassignedReporter, illegalFinalReporter);
    ...
    private static FlowState analyzeDoWhileStatement(
        ...
        FlowState bodyState = analyzeNode(context, children.getFirst(), state, unassignedReporter,
            illegalFinalReporter);
    ...
    private static FlowState analyzeForStatement(
        ...
        if (JAVA_BASIC_FOR_STATEMENT.equals(header.kind().id()))
            return analyzeBasicForStatement(context, header, body, state, unassignedReporter, illegalFinalReporter,
                loopLabel)
        ...
        FlowState bodyState = body == null
            ? headerState
            : analyzeNode(context, body, headerState, unassignedReporter, illegalFinalReporter);
    ...
    private static FlowState analyzeBasicForStatement(
        ...
        FlowState initState = analyzeNodeSequence(context, segments.initNodes(), state, unassignedReporter,
            illegalFinalReporter);
        FlowState loopHead = stabilizeBasicForLoopHead(context, segments, body, initState, unassignedReporter,
            illegalFinalReporter, loopLabel);
        FlowState conditionState = analyzeNodeSequence(context, segments.conditionNodes(), loopHead, unassignedReporter,
            illegalFinalReporter);
        FlowState bodyState = body == null
            ? conditionState
            : analyzeNode(context, body, conditionState, unassignedReporter, illegalFinalReporter);
    ...
    private static FlowState stabilizeWhileLoopHead(
        ...
        for (int i = 0; i < LOOP_FIXPOINT_ITERATION_LIMIT; i++) {
            ...
            FlowState bodyState = body == null
                ? conditionState
                : analyzeNode(context, body, conditionState, null, null);
            FlowState nextHead = FlowState.mergeBranches(baseHead,
                bodyState.resumeContinues(loopLabel).withoutAbruptExits());
    ...
    private static FlowState analyzeTryStatement(
        ...
        FlowState merged = tryBlock == null
            ? current
            : analyzeNode(context, tryBlock, current, unassignedReporter, illegalFinalReporter);
    ...
    private static FlowState analyzeSwitchStatement(
        ...
        FlowState selectorState = analyzeNode(context, children.getFirst(), state, unassignedReporter,
            illegalFinalReporter);
        List<SyntaxNode> rules = directChildrenOfKind(node, JAVA_SWITCH_RULE);
        if (rules.isEmpty())
            return selectorState;

        FlowState switchExit = switchHasDefault(rules)
            ? selectorState.withNormalFlow(false)
            : selectorState.withoutAbruptExits();
        ...
        for (SyntaxNode rule : rules) {
            ...
            FlowState entryState = FlowState.mergeBranches(selectorState.withoutAbruptExits(),
                fallthrough.withoutAbruptExits());
            FlowState ruleState = analyzeSwitchRule(context, rule, entryState, unassignedReporter,
                illegalFinalReporter);
    ...
    private static FlowState applyWrite(
        ...
        if (illegalFinalReporter != null && isFinalVariable(context, symbol)
            && state.maybeAssigned().contains(variableKey(symbol))) {
    ...
    private static boolean isTrackedVariable(Symbol symbol) {
        ...
        if (symbol.declaration().map(declaration -> "JAVA_PATTERN".equals(declaration.kind().id())).orElse(false))
            return false;
        if (symbol.declaration().map(declaration -> "JAVA_RECORD_COMPONENT".equals(declaration.kind().id()))
            .orElse(false))
            return false;
    ...
    private static boolean hasFiniteBasicForExit(BasicForSegments segments) {
        if (segments.conditionNodes().isEmpty())
            return false;
        return segments.conditionNodes().size() != 1
            || !isConstantTrueExpressionText(segments.conditionNodes().getFirst());
    }
    ...
    private static FieldFlowState analyzeFieldIf(
        ...
        FieldFlowState thenState = children.size() > 1
            ...
            : state;
        FieldFlowState elseState = children.size() > 2
            ? analyzeFieldFlow(context, children.get(2), fieldSymbol, state, reporter)
            : state;
    ...
    private static FieldFlowState analyzeFieldTry(
        ...
        FieldFlowState merged = tryBlock == null
            ? state
            : analyzeFieldFlow(context, tryBlock, fieldSymbol, state, reporter);
        for (SyntaxNode catchClause : directChildrenOfKind(node, JAVA_CATCH_CLAUSE)) {
            SyntaxNode catchBlock = context.directChild(catchClause, JAVA_BLOCK);
            if (catchBlock != null) {
                merged = FieldFlowState.merge(merged,
                    analyzeFieldFlow(context, catchBlock, fieldSymbol, state, reporter));
            }
        }
        SyntaxNode finallyClause = context.directChild(node, JAVA_FINALLY_CLAUSE);
        if (finallyClause != null) {
    ...
    private static FieldFlowState analyzeFieldSwitch(
        ...
        for (SyntaxNode rule : rules) {
            ...
            FieldFlowState entryState = FieldFlowState.merge(state.withoutAbruptExits(),
                fallthrough.withoutAbruptExits());
    ...
    private static FieldFlowState analyzeFieldWhile(
        ...
        if (children.isEmpty())
            return state;
        SyntaxNode body = children.size() > 1 ? children.get(1) : null;
        FieldFlowState loopHead = stabilizeFieldWhileLoopHead(context, conditionChild(children), body, fieldSymbol,
            state, reporter, loopLabel);
        FieldFlowState bodyState = body == null
            ? loopHead
            : analyzeFieldFlow(context, body, fieldSymbol, loopHead, reporter);
    ...
    private static FieldFlowState analyzeFieldFor(
        ...
        SyntaxNode header = children.getFirst();
        if (JAVA_BASIC_FOR_STATEMENT.equals(header.kind().id()))
            return analyzeFieldBasicFor(context, header, children.size() > 1 ? children.get(1) : null, fieldSymbol,
                state, reporter, loopLabel);
        SyntaxNode body = children.size() > 1 ? children.get(1) : null;
    ...
    private static FieldFlowState analyzeFieldBasicFor(
        ...
        FieldFlowState initState = analyzeFieldNodeSequence(context, segments.initNodes(), fieldSymbol, state,
            reporter);
        FieldFlowState loopHead = stabilizeFieldBasicForLoopHead(context, segments, body, fieldSymbol, initState,
            reporter, loopLabel);
        FieldFlowState conditionState = analyzeFieldNodeSequence(context, segments.conditionNodes(), fieldSymbol,
            loopHead, reporter);
        FieldFlowState bodyState = body == null
            ? conditionState
            : analyzeFieldFlow(context, body, fieldSymbol, conditionState, reporter);
    ...
    private static FieldFlowState stabilizeFieldWhileLoopHead(
        ...
        for (int i = 0; i < LOOP_FIXPOINT_ITERATION_LIMIT; i++) {
            FieldFlowState bodyState = body == null
                ? loopHead
                : analyzeFieldFlow(context, body, fieldSymbol, loopHead, null);
            FieldFlowState nextHead = FieldFlowState.merge(baseHead,
                bodyState.resumeContinues(loopLabel).withoutAbruptExits());
            if (nextHead.withoutAbruptExits().equals(loopHead.withoutAbruptExits()))
                return nextHead;
            loopHead = nextHead;
    ...
    private static FieldFlowState stabilizeFieldBasicForLoopHead(
        ...
        for (int i = 0; i < LOOP_FIXPOINT_ITERATION_LIMIT; i++) {
            ...
            FieldFlowState bodyState = body == null
                ? conditionState
                : analyzeFieldFlow(context, body, fieldSymbol, conditionState, null);
    ...
    private record FlowState(
        ...
        private FlowState resumeExits(List<FlowExit> exits, boolean clearingBreaks, @Nullable String label) {
            FlowState resumed = reachable
                ? new FlowState(definitelyAssigned, maybeAssigned, true, List.of(), List.of())
                : null;
            ...
            for (FlowExit exit : exits) {
                ...
                var branch = new FlowState(snapshot.definitelyAssigned(), snapshot.maybeAssigned(), true, List.of(),
                    List.of());
    ...
    private record FieldFlowState(
        ...
        private static FieldFlowState merge(FieldFlowState left, FieldFlowState right) {
            ...
            if (!left.reachable && !right.reachable)
                return new FieldFlowState(left.definitelyAssigned || right.definitelyAssigned,
                    left.maybeAssigned || right.maybeAssigned, false, breaks, continues);
            if (!left.reachable)
            ...
            return new FieldFlowState(left.definitelyAssigned && right.definitelyAssigned,
                left.maybeAssigned || right.maybeAssigned, true, breaks, continues);
        }
        ...
        private static FieldFlowState loopExitFrom(@Nullable FieldFlowSnapshot normalExit, FieldFlowState bodyEnd) {
            ...
            if (exits.isEmpty())
                return new FieldFlowState(bodyEnd.definitelyAssigned, bodyEnd.maybeAssigned, false, List.of(),
                    List.of());
