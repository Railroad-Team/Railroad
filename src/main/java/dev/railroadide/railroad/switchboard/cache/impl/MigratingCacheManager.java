        ...
        executor.submit(() -> {
            try {
                if (!(oldBackend instanceof IterableCacheManager iterableOld)) {
                    Railroad.LOGGER.warn("Old cache backend does not support iteration, skipping sweep.");
                    return;
                }
                ...
                for (CacheEntryWrapper entry : iterableOld.entries()) {
                    try {
                        ...
                        Railroad.LOGGER.error("Failed to migrate cache entry: {}", entry.key(), exception);
                    }
                ...
                Railroad.LOGGER.info("Cache migration sweep complete.");
            } catch (Exception exception) {
                Railroad.LOGGER.error("Cache migration sweep failed", exception);
            } finally {
                executor.shutdown();
            }
        });
